...
class ProfessorView(ThemedTk):
    ...
    def show(self):
        self.title(f"Welcome, {self.professor.name}")
        ...
        self.prerequisites_dropdown['values'] = [course.name for course in self.professor.courses]
    ...
    def add_course(self):
        ...
        prerequisites = [course for course in self.professor.courses if course.name == self.prerequisites_dropdown.get()]

        result, message = CourseController.save(name, prerequisites, self.professor.id, capacity)
        if result:
